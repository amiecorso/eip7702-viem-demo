...
async function main() {
  try {
    ...
    if (txnReceipt.status === "success") {
      console.log("✓ Upgrade+Initialization transaction confirmed");
    } else {
      console.log("✗ Upgrade+Initialization transaction failed");
      throw new Error("Upgrade+Initialization transaction failed");
    }
